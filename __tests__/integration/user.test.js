...
describe("Testes de Usuario", () => {
  describe("Testando POST", () => {
    it("Listando usuários com o banco vazio", () => {
      req.get("/api/v1/users").then((result) => {
        result.expect(400);
        result.expect(res.body).toStrictEqual({
    ...
    it("Criando usuário - 1", () => {
      req
        ...
        .then((result) => {
          result.expect(201);
          result.sexpect(res.body).toStrictEqual({
            email: "carlos123@gmail.com",
            senha: "12345",
          });
        });
  ...
  describe("Testando GET", () => {
    it("Listando usuário - 1", () => {
      req.get("/api/v1/users/1").then((result) => {
        result.expect(200);
        result.expect(res.body).toStrictEqual({
          email: "carlos123@gmail.com",
          senha: "12345",
    ...
    it("Listando todos usuários", () => {
      req.post("/api/v1/users/1").then((result) => {
        result.expect(200);
        result.expect(res.body).toStrictEqual([
          {
            email: "carlos123@gmail.com",
  ...
  describe("Testando DELETE", () => {
    it("Deletando usuário - 1", () => {
      req.delete("/api/v1/users/1").then((result) => {
        result.expect(200);
        result.expect(res.body).toStrictEqual({
          email: "carlosAtualizado@gmail.com",
          senha: "12345",
    ...
    it("Deletando usuários", () => {
      req.delete("/api/v1/users").then((result) => {
        result.expect(200);
        result.expect(res.body).toStrictEqual([
          {
            email: "carlosAtualizado2@gmail.com",
